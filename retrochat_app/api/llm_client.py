...
from retrochat_app.api.providers import get_provider_handler, BaseProvider
...
class LLMClient:
    ...
    def _get_provider(self) -> BaseProvider:
        """Return the provider handler for the current configuration."""
        return get_provider_handler(self.config)
    ...
    def send_chat_message_full_history(self, messages: List[Dict[str, Any]]) -> Optional[str]:
        ...
        payload = provider.build_payload(messages, stream=False)
    ...
    def stream_chat_message(self, messages: List[Dict[str, Any]]):
        ...
        headers = provider.build_headers()
        payload = provider.build_payload(messages, stream=True)
        endpoint = provider.endpoint()
        ...
        try:
            with requests.post(
                ...
                response.raise_for_status()
                for text in provider.iter_stream(response.iter_lines()):
                    yield text
        except requests.exceptions.RequestException as e:
            logger.error(f"Error during LLM communication: {e}")
            yield ""
